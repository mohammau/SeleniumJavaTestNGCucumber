...
public class LoginUser extends Base {
	...
	public void Login(String UN, String Password, String Expected) {
		...
		LoginSignUpPage sr1 = new LoginSignUpPage(driver);
		AssertJUnit.assertTrue(sr1.signinHeader().isDisplayed());
		sr1.loginEmailField().sendKeys(UN);
		sr1.loginPasswordField().sendKeys(Password);
		
		sr1.loginButton().click();
			
		
		
		LoggedInUser lup = new LoggedInUser(driver);
		String actual = null;
